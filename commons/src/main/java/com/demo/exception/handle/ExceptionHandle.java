...
public class ExceptionHandle {
    ...
    public ResponseEntity<ApiError> handleBaseException(ApplicationException ex, WebRequest request) {
        log.error(ExceptionUtils.getMessage(ex));
        ApiError errorModel = new ApiError(ex.getHttpStatus().value(), ex.getMessages(), ex.getErrorCode());
...
//        }
    ...
    public ResponseEntity<Object> handleBindException(Exception ex, WebRequest request) {
        ...
        List<String> messages = customEx.getFieldErrors().stream()
                ...
                .collect(Collectors.toList());
        ApiError errorModel = new ApiError(httpStatus.value(), messages, null);
        return new ResponseEntity<>(errorModel, header(), httpStatus);
    }

//    @ExceptionHandler({MultipartException.class})
//    public ResponseEntity<Object> handleMultipartException(Exception ex, WebRequest request) {
//        if (ex.getCause() instanceof IllegalStateException && ex.getCause().getCause() instanceof RequestTooBigException) {
//            HttpStatus httpStatus = HttpStatus.BAD_REQUEST;
//            ErrorModel errorModel = new ErrorModel(
//                    httpStatus.value(),
//                    "Vượt quá kích thước request tối đa"
//            );
//
//            return new ResponseEntity<>(errorModel, header(), httpStatus);
//        }
